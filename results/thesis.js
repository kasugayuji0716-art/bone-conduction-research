...
const content = [
  ...
  p([txt("また、再構成損失（L1+STFT）の必要性を検証するablation実験を行った。CE損失のみ（再構成損失なし、"), mi("\u03BB"), txt(" = 10.0）で学習した場合のtest CERは0.205であり、TAPS pretrained（0.252）を上回るものの、再構成損失を加えた場合（0.200）と比較すると劣る。両損失は相補的に機能している。")]),

  h2("4.2 別ASRモデルでの評価"),
  p([txt("表2に、Whisper-smallで学習したCE-aware SEを異なるサイズのWhisperモデルで評価した結果を示す。提案手法（CE "), mi("\u03BB"), txt(" = 10.0）は全3モデルでTAPS pretrainedを上回っており、学習に使用したWhisper-small以外のサイズにも改善が転移した。一方、encoder距離損失（Enc L1）はTAPS pretrainedとほぼ同等のCERにとどまり、明確な改善は得られなかった。")]),
  tbl(
  ...
  p([txt("一方、Enc L1はWhisper-smallのencoder距離を縮小（0.200→0.192）しているが、CERの改善は限定的である（0.252→0.251）。encoder距離の縮小がCERの大幅改善につながらないことは、距離縮小とCER改善が単調に対応しないことを意味する。")]),
  ...
  p([txt("第一に、提案手法（CE "), mi("\u03BB"), txt(" = 10.0）はTAPS pretrainedベースラインと比較してCERを20.7%改善し（0.252→0.200、"), mi("p"), txt(" = 3.38\u00D710"), sup("-93"), txt("）、統計的に極めて有意な差を達成した。第二に、この改善は学習に使用したWhisper-small以外のサイズ（base・medium）にも転移した。Enc L1はTAPSとほぼ同等にとどまるのに対し、CE損失は全サイズで明確な改善を示した。第三に、encoder距離分析（表3）により、CE損失が気導音声への復元とは異なるメカニズムでASR精度を改善していることが明らかになった。CE損失で学習したSEはencoder空間において気導音声からむしろ離れるが、CERは大幅に改善しており、encoder L1距離の低下だけでは改善を説明できないことが示唆された。")]),

  p([txt("一方、提案手法のCER 0.200はWhisperファインチューニング（CER 0.136）には及ばない。しかし、SEはASRモデルに依存しない前処理であり、今回評価した3サイズには同一SEを再学習なしで適用できた。本研究はWhisper系列内での評価に限られており、異なるASRアーキテクチャや他言語への汎化は今後の検証課題である。")]),
