...
const content = [
  ...
  p([txt("しかし、頸部の軟組織を介した信号伝播は物理的なローパスフィルタとして機能し、3〜4 kHz以上の高域成分が大幅に減衰する。この帯域制限により、喉マイク音声は気導マイク音声と比較して音声品質が低下し、自動音声認識（Automatic Speech Recognition; ASR）の精度が著しく劣化する。例えば、TAPSデータセット[4]においてWhisper-smallで韓国語喉マイク音声を認識した場合のCER（Character Error Rate）は0.47であり、気導マイク音声と比較して大幅に高い。")]),

  p([txt("この課題に対し、音声強調（Speech Enhancement; SE）を前処理として適用する手法が研究されてきた。しかし近年、Ochiaiら[1]は気導マイクの文脈において、SEがASR精度を改善するどころか悪化させる場合があることを報告した。彼らは既存の直交射影に基づく分解（Orthogonal Projection-based Decomposition; OPD）を用いたDSA（Distortion Source Analysis）により、SEの推定信号をtarget成分・interference error・noise error・artifact errorに分解し、artifact errorが逆効果の主因であることを特定した。Mawalimら[2]も、STOI（Short-Time Objective Intelligibility）やPESQ（Perceptual Evaluation of Speech Quality）が改善しているにも関わらずASR精度が悪化する現象を報告している。")]),

  p([txt("この「知覚品質の改善がASR精度の改善を保証しない」という問題の一因は、従来のSEが波形品質の復元（L1損失やSTFT損失）を最適化目標としており、ASR精度の最適化が間接的にしか行われない点にある。Closeら[3]は自己教師あり音声表現の特徴距離をSE損失に利用するPerceive & Predictを提案し、知覚品質指標との相関を示したが、実際のASR性能（CER）への効果は評価されていない。")]),
  ...
  p([txt("(1) 提案手法はベースラインSEのCERを20.7%改善した（0.252→0.200）。")]),
  p([txt("(2) 学習に使用していないWhisper系列の別サイズモデルでも改善が転移した。")]),
  ...
  p([txt("提案手法では、上記ベースラインの重みを初期化点とし、再構成損失にASR損失を加えた損失関数で追加学習（fine-tuning）を行う。再構成損失はTAPS論文と同じ構成・時間周波数設定のL1＋多解像度STFT損失であり、波形品質を維持する正則化として機能する。")]),
  p([
  ...
  p([txt("encoder距離損失が中間表現の類似度という間接的な目標を最適化するのに対し、CE損失は正解テキストに対するtoken単位の交差エントロピーを最小化する点が本質的に異なる。")]),
  ...
  h2("3.1 データセット"),
  p([txt("韓国語喉マイクデータセットTAPS（Throat and Acoustic Paired Speech Dataset）[4]を使用した。TAPSは喉マイクと気導マイクのペア同時収録データセットであり、60話者・6,000発話から構成される。話者独立な分割設計（train: 40話者・4,000発話、dev: 10話者・1,000発話、test: 10話者・1,000発話）が採用されている。喉マイクは8 kHzで収録され16 kHzにアップサンプリングされている。")]),

  h2("3.2 SEモデルと学習設定"),
  p([
    txt("TAPS論文で提供されたSE-Conformer（12.1Mパラメータ）の事前学習済み重みから初期化し追加学習を行った。15秒を超える発話は音声とテキストの対応を維持するため除外した（train: 78件除外、dev: 9件除外）。最適化にはAdam（lr=3\u00D710"),
    sup("-4"),
    txt("、\u03B2=(0.9, 0.99)）を使用し、batch size=4、最大50エポック、early stopping（patience=5）で学習した。CE損失のWhisper forward部分にはAMP（混合精度演算）を適用した。"),
  ]),

  h2("3.3 評価設定"),
  p([txt("ASR評価にはfaster-whisper（CTranslate2形式）を使用し、言語=韓国語、beam size=5でCERを計測した。CERは発話ごとにjiwer.cerで計算し1.0でキャップした後、全発話の算術平均をとった（発話平均capped CER）。汎用性検証としてWhisper-base（74M）・small（244M）・medium（769M）の3モデルで評価した。知覚品質はpySTOIによるSTOIとITU-T P.862に基づくPESQ（広帯域モード）で計測し、同一発話の気導マイク音声を参照信号とした。")]),
  ...
  h2("4.1 CE損失の\u03BB探索"),
  p([
    txt("表1にCE損失の重み"), mi("\u03BB"), txt("とCERの関係を示す。"), mi("\u03BB"), txt("の選択はdev CERに基づいて行い、testセットは最終評価のみに使用した。"), mi("\u03BB"), txt(" = 10.0がdev上で最良であり、対応するtest CERは0.200で、TAPS pretrainedベースラインと比較して20.7%の改善となった。Wilcoxon符号順位検定の結果、"),
    mi("p"), txt(" = 3.38\u00D710"),
    sup("-93"),
    txt("であり統計的に極めて有意な差が確認された。話者内の依存を考慮し話者平均を単位とした検定（"), mi("n"), txt(" = 10）でも"), mi("p"), txt(" = 0.002であり有意な差が確認された。"),
  ]),
  tbl(
    ...
    [
      ...
      ["CE \u03BB=10.0（best）", "0.232", "0.200", "-20.7%"],
    ],
    [1200, 1000, 1000, 1000]
  ),
  ...
  cap("表2: Whisper系列の異なるサイズでのCER比較"),

  h2("4.3 Encoder距離分析"),
  p([txt("CE損失の改善メカニズムを分析するため、各SE条件について3つのWhisperモデルのencoder表現間L1距離を計測した（表3）。SE出力と気導音声を各encoderに入力し、出力表現のL1距離を計算した。")]),
  tbl(
  ...
  cap("表3: 各Whisperモデルのencoder表現間L1距離"),

  p([txt("注目すべきは、CE損失（CER最良）がTAPS pretrainedよりもencoder距離が"),
    txtB("大幅に大きい"),
    txt("点である（small: 0.200→0.326）。CE損失で学習したSEの出力は気導音声にencoder空間で近づいておらず、むしろ離れている。それにも関わらずCERは大幅に改善している（表2）。これは、encoder表現を気導音声に近づけることがASR精度向上の必要条件ではないことを示す。")]),
  p([txt("一方、Enc L1はWhisper-smallのencoder距離を縮小（0.200→0.192）しているが、CERの低下は限定的である（0.252→0.251）。CEは距離が増加しつつCERが大幅低下しており、条件間でencoder距離の順位とCERの順位が一致しない。")]),
  p([txt("以上の結果は、気導参照へのencoder L1距離の低下だけではCER改善を説明できないことを示唆する。CE損失は気導音声に似た表現を生成するのではなく、ASRの書き起こし誤りを減少させる別の表現を生成している可能性がある。")]),

  h2("4.4 知覚品質評価"),
  p([txt("表4にSTOI・PESQ・CERの比較を示す。TAPS pretrainedはSTOI 0.892・PESQ 1.975と最も高いSTOI・PESQ値を達成するが、CERは0.252にとどまる。一方、提案手法（CE "), mi("\u03BB"), txt(" = 10.0）はSTOI 0.792・PESQ 1.215とSTOI・PESQが低下するにも関わらず、CERは0.200と最良である。")]),
  tbl(
    ["条件", "STOI", "PESQ", "CER"],
    [
