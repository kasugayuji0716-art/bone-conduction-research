...
// 2. 問いと手法
{
  ...
  T(s, [{ text: "ベースライン：", options: { bold: true } }, { text: "TAPS論文の学習済みSE（以下 TAPS SE）", options: { breakLine: true } },
         { text: "データ：", options: { bold: true } }, { text: "韓国語TAPS（喉マイク／気導マイクの同時収録）", options: { breakLine: true } },
         { text: "評価：", options: { bold: true } }, { text: "雑音のない収録音声、テスト話者10名" }],
    { x: rx + 0.33, y: 3.85, w: rw - 0.66, h: 2.2, fontSize: 13, color: C.sub, lineSpacingMultiple: 1.6 });
...
// 4. わかったこと
{
  const s = pptx.addSlide(); s.background = { color: C.light };
  title(s, "わかったこと");
  T(s, "比較の相手はすべて TAPS SE（ASR損失なしで学習した元のSE）", { x: M, y: 1.3, w: W, h: 0.35, fontSize: 13, color: C.sub });
  const rows = [
    ...
    ["出力の音", C.dark, rich(["STOI・PESQは下がり、250 Hz間隔の人工的な成分が現れる。気導音声らしい音ではなく、", { b: "Whisperが読みやすい音" }, "を作っている"])],
  ];
  let y = 1.9; const rh = 1.5;
  rows.forEach(([k, kc, v], i) => {
    s.addShape(pptx.ShapeType.line, { x: M, y, w: W, h: 0, line: { color: C.line, width: 1 } });
    T(s, k, { x: M, y: y + 0.12, w: 3.0, h: rh - 0.24, fontSize: 19, bold: true, color: kc, valign: "middle" });
    T(s, v, { x: M + 3.2, y: y + 0.12, w: W - 3.2, h: rh - 0.24, fontSize: 19, lineSpacingMultiple: 1.35, valign: "middle" });
    y += rh;
  });
  s.addShape(pptx.ShapeType.line, { x: M, y, w: W, h: 0, line: { color: C.line, width: 1 } });
  s.addNotes("【台本】\nわかったことです。細かい数字は省いて、傾向だけお話しします。比べている相手は、すべて元のTAPSのSEです。\n\n（1行目）まず、学習に使ったWhisper-smallでは、CERは確かに改善しました。テスト用の10話者全員で改善しています。同じWhisper系列のmediumにも改善は持ち越されましたが、小さいbaseでははっきりした差が出ませんでした。\n\n（2行目）一方で、Whisperとは作りが違うwav2vec2系の音声認識では改善しませんでした。XLS-Rでははっきり悪化し、MMSではほぼ変わりませんでした。\n\n（3行目）最後に出力の音です。STOIやPESQといった音の品質の指標は、むしろ下がっていました。さらに出力には、250Hz間隔で鋭いピークが並ぶ人工的な成分が現れていました。気導マイクの音にはない成分です。つまり、気導マイクらしい自然な音に近づけているのではなく、Whisperが読みやすい別の音を作っている、ということです。\n\n（補足：聞かれたら）Whisper-smallのCERは、TAPSのSEの0.230から0.196で、15%の改善です。XLS-Rは約26%悪化しています。\n\n（目安：1分15秒）\n\n【想定質問】\nQ. CERの計算で気をつけたことは？\nA. TAPSの正解テキストには句読点がありません。ASR損失で学習したSEはWhisperが句読点を出さなくなる音を作るので、そのままだと句読点の分が改善に見えてしまいます。そこで正解と認識結果の両方から句読点を除いてからCERを計算しています。上の数字はその値です。");
}
...
// 5. 現状の評価
{
  ...
  const cards = [["新しさが小さい", "凍結Whisperの損失で前処理を学習する手法はDissen 2024が既出。ASR損失で学習したSEが他のASRで悪化することも、Sato 2025・D4AM 2023が報告済み"],
...
// 6. 行き詰まっていない部分
{
  ...
  const cards = [["分析の成果", C.dark, C.blue, rich(["「汎用性を失う」は先行研究と同じ傾向。", { b: "新しいのは" }, "、帯域が欠けた喉マイクでの確認と、出力に現れる250 Hz間隔の人工成分の分析"]), "→ 卒論・学会発表には十分"],
  ...
  s.addNotes("【台本】\nただ、研究そのものが行き詰まったわけではないと考えています。\n\n（左のカード）1つ目に、分析の成果としての価値があります。ASR損失で学習するとSEの汎用性が失われること、そしてこういうSEを評価するには、テキストの正規化と、学習に使っていない系列のASRでの評価が必要だということです。さっきお話ししたとおり、「汎用性を失う」こと自体は先行研究と同じ傾向です。新しいと言えるのは、4kHz以上が欠けた喉マイクという条件で確かめたことと、出力に現れる250Hz間隔の人工的な成分を分析したことです。卒論や学会発表としては、この分析でまとめられると考えています。\n\n（右のカード）2つ目、こちらの方が大きいのですが、次に何を目指すべきかがはっきりしました。SEの長所である汎用性を保ったまま、ASRに効くSEをどう作るか、です。\n\n（目安：1分）\n\n【想定質問】\nQ. 先行研究と同じ結論なら、卒論の新規性は？\nA. 喉マイクへの適用と、出力に現れる250Hz間隔の人工的な成分の分析が新しい部分です。");
}
...
// 7. 次の方向：本命
{
  ...
  const rows = [
    ...
    ["手がかり", rich(["自己教師あり表現（WavLMなど）を損失に使うと、", { b: "学習に使っていないASRでも改善した" }, "という報告がある（Sato 2025、Ravenscroft 2024）。特定のASRの出力ではなく、「音声の表現」に近づける方向"])],
    ["評価の枠組み", rich(["学習に使っていない系列のASRで、話者ごとに評価する。", { b: "卒論で作ったものがそのまま使える" }])],
  ];
  let y = 2.2;
  rows.forEach(([k, v]) => {
    const h = 1.28;
  ...
  s.addNotes("【台本】\n次の方向の本命として考えているのは、特定のASRに依存しない、喉マイク用のSEです。\n\n（動機）実際の現場では、クラウドのAPIや、他のサービスと共用しているASRのように、重みを変えられないASRが多いです。その場合は、ファインチューニングも、今回のようなASR損失での学習もできません。つまり、ASRを選ばないSEしか選択肢がない。ここが、ファインチューニングに対するSEの本当の存在意義だと考えています。\n\n（手がかり）手がかりは先行研究にあります。WavLMのような自己教師あり学習モデルの表現を損失に使ってSEを学習すると、学習に使っていないASRでも改善した、という報告があります。NTTのSatoらの2025年の論文や、Ravenscroftらの2024年の研究です。特定のASRの出力に合わせるのではなく、音声の表現そのものに近づける方向なので、汎用性を保てる可能性があります。\n\n（評価の枠組み）評価については、学習に使っていない系列のASRで、話者ごとに評価する、といった枠組みを卒論で作ったので、それがそのまま使えます。\n\n（目安：1分15秒）\n\n【想定質問】\nQ. 自分の実験で、表現に近づける損失は試していないのか？\nA. 比較手法として、気導マイクの音をWhisperに入れたときの表現に近づける損失（Enc L1）を試しました。Whisper以外のASRで悪化はしませんでしたが、どのASRでも改善もせず、元のTAPS SEからほとんど変わりませんでした。なので、Whisperの表現ではなく、自己教師あり学習モデルの表現を使うところから試したいと考えています。");
}
...
// 8. 次の方向：その他
{
  ...
  const data = [["複数のASRの損失を混ぜて学習", "自然な発展", "平均的になるだけかも。学習に使っていない系列には効かない可能性"],
                ["雑音のある環境で評価（今回は雑音なし）", "喉マイク本来の強み（騒音に強い）を活かせる", "データの収録が必要。競合研究がある"],
                ["SEとFTがそれぞれ何を直しているかの分析", "分析として面白い", "手法の提案にはならない"]]
  ...
  s.addShape(pptx.ShapeType.roundRect, { x: M, y: 5.95, w: W, h: 0.65, fill: { color: C.dark }, line: { color: C.dark }, rectRadius: 0.08 });
  T(s, "ご意見をいただきたい点：本命の方向でよいか ／ ほかに検討すべき選択肢はあるか", { x: M + 0.25, y: 5.95, w: W - 0.5, h: 0.65, fontSize: 16, bold: true, color: C.light, valign: "middle" });
  s.addNotes("【台本】\n本命以外にも、考えられる選択肢を3つ挙げます。\n\n（1行目）1つ目は、複数のASRの損失を混ぜて学習する方法です。自然な発展ですが、平均的な結果になるだけで、学習に使っていない系列には効かない可能性があります。\n\n（2行目）2つ目は、雑音のある環境で評価することです。今回の実験は雑音のない収録音声だけでした。喉マイクの本来の強みは騒音に強いことなので、それを活かせます。ただ、データを自分で収録する必要がありますし、競合する研究もあります。\n\n（3行目）3つ目は、SEとファインチューニングが、それぞれ音声のどこを直しているのかを分析する方向です。分析としては面白いのですが、手法の提案にはなりません。\n\n以上です。卒論は分析としてまとめ、この先は本命の方向に切り替えたいと考えています。（下の帯を指して）この本命の方向でよいか、また他に考えるべき選択肢があるか、ご意見をいただけるとありがたいです。ありがとうございました。\n\n（目安：1分）");
}
