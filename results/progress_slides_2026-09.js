...
// 6. 行き詰まっていない部分
{
  ...
  const cards = [["分析の成果", C.dark, C.blue, rich(["「汎用性を失う」は先行研究と同じ傾向。", { b: "新しいのは" }, "、帯域が欠けた喉マイクでの確認、句読点による見かけの改善の定量化、250 Hz間隔の人工成分の分析"]), "→ 卒論・学会発表には十分"],
  ...
  s.addNotes("【台本】\nただ、研究そのものが行き詰まったわけではないと考えています。\n\n（左のカード）1つ目に、分析の成果としての価値があります。ASR損失で学習するとSEの汎用性が失われること、そしてこういうSEを評価するには、テキストの正規化と、学習に使っていない系列のASRでの評価が必要だということです。さっきお話ししたとおり、「汎用性を失う」こと自体は先行研究と同じ傾向です。新しいと言えるのは、4kHz以上が欠けた喉マイクという条件で確かめたこと、句読点による見かけの改善を定量化したこと、そして出力に現れる250Hz間隔の人工的な成分を分析したことです。卒論や学会発表としては、この分析でまとめられると考えています。\n\n（右のカード）2つ目、こちらの方が大きいのですが、次に何を目指すべきかがはっきりしました。SEの長所である汎用性を保ったまま、ASRに効くSEをどう作るか、です。\n\n（目安：1分）\n\n【想定質問】\nQ. 先行研究と同じ結論なら、卒論の新規性は？\nA. 喉マイクへの適用と、句読点による見かけの改善の定量化、それに出力に現れる250Hz間隔の人工的な成分の分析が新しい部分です。");
}
