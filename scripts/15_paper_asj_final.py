...
def main():
    ...
    body(doc,
        ...
        '図3上段(A)にBefore/After絶対スペクトル，下段(B)に差分（Δ = After − Before）を示す．'
        '差分スペクトルを帯域平均すると，50 Hz以下の極低域で+15.04 dBという突出した'
        'エネルギー追加が観測された一方，100 Hz以上はすべて削除方向であり，'
        '特に4–8 kHz帯では−12.73 dBという壊滅的な高域削除が生じた．'
        '1–4 kHz帯（フォルマントの主要周波数帯）でも−2.23 dBの抑制が確認された．')
    body(doc,
        '喉マイクは本来，骨振動の伝達特性により2 kHz以上の高周波成分が構造的に欠落している．'
        'DNS3（気導マイク・英語）で学習したGTCRNは，この高域の無音状態を「残留ノイズ」と誤判定し，'
        'さらに除去しようとする逆方向の動作をとる．'
        '50 Hz以下への低域アーティファクト追加はASRの音韻識別に影響しない帯域であるが，'
        '1–8 kHz帯の音韻情報の系統的な損失がCER悪化の直接的原因である．'
        'STOI・PESQは知覚上の了解度・音質を反映する一方，'
        'このような高域抑制によるASRへの悪影響を検出できないことが三重パラドックスの本質である．')
    ...
    body(doc,
        ...
        'スペクトル分析（3.4節）はこのメカニズムを周波数レベルで直接可視化した：'
        'GTCRNは50 Hz以下の極低域に+15 dBのアーティファクトを追加する一方，'
        '100 Hz以上はすべて削除方向であり，4–8 kHz帯は−12.73 dBと壊滅的に抑制される．'
        '1–4 kHz帯（フォルマント主要帯域）も−2.23 dBと系統的に削減される．'
        '喉マイクが既に欠落している高域をさらに除去するこの逆方向の動作が，'
        'Ochiaiらのアーティファクト誤差フレームワークにおける「処理歪み」の実態である．'
    ...
    body(doc,
        '本稿の主要な貢献は以下の3点である．'
        '（1）SE逆効果パラドックス（STOI↑PESQ↑CER↑）を，高周波成分が構造的に欠落した'
        '喉マイクというドメインで系統的に実証した（30条件中26件 p<0.05）．'
        'Ochiaiら [2] やMawalimら [1] による先行報告は気導マイクを対象としており，'
        '喉マイク特有のドメインシフト条件での検証は本研究が初めてである．'
        '（2）スペクトル分析によりGTCRNが4–8 kHz帯を−12.73 dBと削除するという'
        '具体的なアーティファクト機構を実測で特定した．'
        'この「既に欠落している高域をさらに除去する逆方向の動作」という描像は，'
        'Ochiaiらのフレームワークを喉マイクドメインに接地させる新たな知見である．'
        f'（3）ASRモデルのドメイン適応（Whisper small FT）により'
        f'CERを{ms_A:.3f}→{ms_C:.3f}（{ms_ft_improve:.1f}%改善，10話者・1,000発話）と根本的に改善し，'
        f'FT後はSEの悪影響も縮小（{ms_se_harm_pre:+.3f}→{ms_se_harm_ft:+.3f}）することを示した．'
        '喉マイクASRの改善にはSEの知覚品質最適化よりもASRドメイン適応が本質的に有効であり，'
        'STOI・PESQを評価指標として用いることの限界も定量的に示す．')
    body(doc,
        '本研究の限界として，以下の3点を挙げる．'
        '第一に，使用したSEモデル（GTCRN）はドメイン外学習済みモデルであり，'
        '喉マイク向けに再設計・再学習されたSEとの比較は行っていない．'
        '第二に，評価データはTAPS（韓国語）のみであり，他言語・他データセットへの汎化性は未検証である．'
        '第三に，Whisper FTはクリーン音声で実施しており，ノイズ下での頑健性評価が不足する．'
        '今後の課題として，TAPSで学習したドメイン適合型SE-conformer [5] との組み合わせによる'
        'さらなるCER改善，Whisper large-v3でのFT検証，'
        'および喉マイクで特に誤認識されやすい音素の分析を予定する．')
