"""
スクリプト52: CE損失モデル vs encoder距離モデルのCER比較評価

使い方（DNN PC）:
    python scripts/52_eval_ce_loss.py
"""
...
CONDITIONS = [
    ...
    ('enc_lambda_5.0',  'Encoder L1 (lambda=5.0)',  BASE_DIR / 'checkpoints' / 'lambda_5.0' / 'best.th'),
    ('ce_lambda_5.0',   'CE loss (lambda=5.0)',     BASE_DIR / 'checkpoints' / 'ce_lambda_5.0' / 'best.th'),
]
...
def main():
    ...
    print(f'\n{"="*55}')
    print(f'{"Condition":<30} {"CER":>8} {"n":>6}')
    print('-' * 55)
    for key, label, _ in conditions:
        m = np.mean(results[key])
        print(f'{label:<30} {m:>8.4f} {len(results[key]):>6}')

    out = BASE_DIR / 'results' / 'ce_loss_comparison.csv'
